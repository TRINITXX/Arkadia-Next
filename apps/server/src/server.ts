...
const ReactorLayerLive = Layer.empty.pipe(
  ...
  // Shared singleton: ingestion publishes on it, every thread subscription
  // reads from it, so it must be merged once for both to see the same PubSub.
  Layer.provideMerge(PromptSuggestionBusLive),
);
