...
// The Hermes driver spawns `ssh` and tunnels `hermes acp` over stdio. In tests
// we point `binaryPath` at a fake ssh that ignores its arguments (the -o flags,
// sudo, bash -lc …) and execs the shared ACP mock agent locally instead.
async function makeMockHermesSsh(extraEnv?: Record<string, string>) {
  const dir = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "hermes-acp-mock-"));
  const wrapperPath = NodePath.join(dir, "fake-hermes-ssh.sh");
...
it.layer(hermesAdapterTestLayer)("HermesAdapterLive", (it) => {
  it.effect("starts a session and maps mock ACP prompt flow to runtime events", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() => makeMockHermesSsh());
